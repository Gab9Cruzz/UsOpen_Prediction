...
def compute_surface_metrics(
    matches: pd.DataFrame, surface: str, cutoff_date: int
) -> pd.DataFrame:
    ...
    for _, m in df.iterrows():
        ...
        if pd.notna(w_serve_total) and pd.notna(l_serve_total):
            _accumulate(
                ...
                serve_won=w_serve_won, serve_total=w_serve_total,
                return_won=l_serve_total - l_serve_won, return_total=l_serve_total,
            )
            _accumulate(
                m["loser_id"], m["loser_name"],
                serve_won=l_serve_won, serve_total=l_serve_total,
                return_won=w_serve_total - w_serve_won, return_total=w_serve_total,
            )
...
def run_ingest(
    ...
    metricas_superficie = metrics[
        [
            ...
            "serve_pts_won", "serve_pts_total", "serve_pct",
            "return_pts_won", "return_pts_total", "return_pct",
        ]
    ].copy()
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute("DELETE FROM metricas_superficie")
        conn.execute(
